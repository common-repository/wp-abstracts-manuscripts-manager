...
<div class="wpabstracts container-fluid">
	<?php
	if(!is_admin() && !wpabstracts_user_can_edit($abstract->status)) { 
	...
	<h3>
		...
		<button type="button" id="abs_submit_btn" onclick="wpabstracts_validateAbstract();" class="wpabstracts btn btn-primary"><?php _e('Submit','wpabstracts');?></button>
	</h3>
			...
			<div class="wpabstracts col-xs-12 col-sm-12 col-md-8">
					...
					<div class="wpabstracts panel-body">
						...
						<?php if (get_option('wpabstracts_show_description')) { ?>
							<div class="wpabstracts form-group">
								...
								$settings = array( 'editor_id' => 'abstext', 'media_buttons' => $editorMedia, 'wpautop' => true, 'editor_height' => 360, 'quicktags' => true);
				...
				<div class="wpabstracts col-xs-12 col-md-4">
					...
					<div class="wpabstracts panel panel-default">
						...
						<div class="wpabstracts panel-body">
							<div class="wpabstracts form-group">
								...
								<?php } ?>
								<div class="wpabstracts form-group">
									<label class="wpabstracts control-label" for="abs_topic_id"><?php echo apply_filters('wpabstracts_title_filter', __('Topic','wpabstracts'), 'topic');?></label>
									<select name="topic_id" id="topic_id" class="wpabstracts form-control wpa_event_input">
										...
										<?php foreach($topics as $topic){ ?>
											<option value="<?php echo esc_attr($topic->topic_id);?>" <?php selected($topic->topic_id, $abstract->topic_id);?>><?php echo esc_attr($topic->name);?></option>
										<?php } ?>
									</select>
								</div>
							</div>
					...
					<?php if(get_option('wpabstracts_show_author')){?>
						...
						<?php $currentUser = wp_get_current_user(); ?>
						<?php if($currentUser->roles[0]=='administrator' || $currentUser->roles[0]=='subscriber' || $currentUser->ID == $abstract->submit_by ||
							($currentUser->roles[0]=='editor' && !get_option('wpabstracts_blind_review'))){ ?>
