...
function wpabstracts_delete_template(id, tab) {
	alertify.confirm(wpabstracts.confirm_template_delete, function (e) {
		if (e) {
			location.href =
				'?page=wpabstracts&tab=' + tab + '&subtab=emails&task=delete&id=' + id;
		}
	});
}
...
function wpabstracts_load_topics(id) {
	...
	jQuery.post(wpabstracts.ajaxurl, data).done(function (data) {
		jQuery('#topic_id').html(data);
	});
}
