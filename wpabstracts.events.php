...
if(is_admin() && isset($_GET['tab']) && ($_GET["tab"]=="events")){
	wpabstracts_events_header();
	$subtab = isset($_GET['subtab']) ? sanitize_text_field($_GET['subtab']) : 'manage';
	wpabstracts_events_tabs($subtab);
}

function wpabstracts_events_tabs($subtab) {
	$admin_page = WPABSTRACTS_PLUGIN_DIR . "events/events." . $subtab . ".php";
...
function wpabstracts_events_header(){

	$current = isset($_GET['subtab']) ? $_GET['subtab']  : 'manage';

	$sections = array(
		'manage' => __('Manage', 'wpabstracts'),
	...
	foreach( $sections as $slug => $name ){
		...
		$sub_menu .= "<li role='presentation' class='submenu-item ".$class."'><a href='?page=wpabstracts&tab=events&subtab=$slug'><strong>$name</strong></a></li>";
	}
	$sub_menu .= '</ul>';
	$sub_menu .= '</div>';
	echo $sub_menu;
